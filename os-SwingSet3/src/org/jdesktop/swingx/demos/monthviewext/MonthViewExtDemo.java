/*
 * $Id: MonthViewExtDemo.java 1299 2013-03-06 13:38:20Z kleopatra $
 *
...
import org.jdesktop.swingxset.util.ResourceMapStringValue;
...
//---------------------- bind
    ...
    private static final Object BASE = new Object();
    private ComboBoxModel createHeaderHandlers() {
        // <snip> Custom CalendarHeaderHandler
        // create combo model containing handlers to choose
        DefaultComboBoxModel model = new DefaultComboBoxModel();
        model.addElement(BASE);
        model.addElement(new SpinningCalendarHeaderHandler());
        model.addElement(new DemoCalendarHeaderHandler(true, true));
        // </snip>
        return model;
    }
    
    private Map<Object, String> createDescriptionLookup(ListModel model, String... descriptions) {
        Map<Object, String> descriptionMap = new HashMap<Object, String>();
        for (int i = 0; i < Math.min(descriptions.length, model.getSize()); i++) {
            descriptionMap.put(model.getElementAt(i), descriptions[i]);
        }
        return descriptionMap;
    }
    
    public static class NullConverter extends Converter<Object, Object> {

        private Object nullObject;

        public NullConverter(Object nullObject) {
...
//--------------------- MonthViewDemoControl
    public class MonthViewDemoControl extends AbstractBean {
        ...
        public MonthViewDemoControl() {
            ...
            System.out.println("property? " + UIManager.get("headerHandler.base"));
            ...
            // PENDING JW: DisplayInfos as data is a hack - the model should 
            // contain the data, not a mixture of data and rendering hints!
//            customHeaderBox.setModel(createHeaderHandlers());
//            Map<Object, String> descriptionMap = createDescriptionLookup(customHeaderBox.getModel(),
//                    new String[]{"base", "default", "custom"});
//            customHeaderBox.setRenderer(new DefaultListRenderer(
//                    new ResourceMapStringValue(descriptionMap, "headerHandler.", MonthViewExtDemo.class)
//                    ));
            ...
            if (customHeaderBox.getItemAt(0) instanceof DisplayInfo) {
                handlerBinding.setConverter(new DisplayInfoConverter<CalendarHeaderHandler>());
            } else {
                handlerBinding.setConverter(new NullConverter(customHeaderBox.getItemAt(0)));
            }
            // </snip>
